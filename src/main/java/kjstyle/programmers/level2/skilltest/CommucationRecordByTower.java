 ...
 */
public class CommucationRecordByTower {

	@Test
	public void test01() {
		int[] heights = { 6, 9, 5, 7, 4 };
		int[] expected = { 0, 0, 2, 2, 4 };
		Assert.assertArrayEquals(expected, this.solution(heights));
	}
		...
		for (int i = SIZE - 1; i >= 0; i--) {
			...
			for (int j = (i - 1); j >= 0; j--) {
				if (heights[j] > heights[i]) {
					answer[i] = j + 1; // 탑번호는 1번부터 시작함. 수신받은 j번째 index에 더하기 1을 해줘야 탑번호가 됨
					break;
				}
			}
		}
